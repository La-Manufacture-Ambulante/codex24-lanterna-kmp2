...
class TextCharacterTest {
    @Test
    fun fromString() {
        assertEquals(6, TextCharacter.fromString("Hello!")!!.size)
        assertEquals(5, TextCharacter.fromString("あいうえお")!!.size)
        assertEquals(1, TextCharacter.fromString("\uD83C\uDF55")!!.size)
        assertEquals(1, TextCharacter.fromString("\uD83E\uDD7A")!!.size)
        // This is a weird compound character that should be printed as one, but is actually two char:s
        assertEquals(1, TextCharacter.fromString("บุ")!!.size)
