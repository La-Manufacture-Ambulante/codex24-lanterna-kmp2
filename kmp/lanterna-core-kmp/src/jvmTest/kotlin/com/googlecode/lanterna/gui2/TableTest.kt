...
class TableTest : TestBase() {
    ...
    fun init(textGUI: WindowBasedTextGUI) {
        ...
        val table = Table<String>("Column 1", "Column 2", "Column 3")
        table.setTableCellRenderer(
            object : DefaultTableCellRenderer<String>() {
            ...
            Button("Add...", {
                ActionListDialogBuilder()
                    ...
                    .addAction("Row", {
                        ...
                        model.addRow(*labels.toTypedArray())
                        table.invalidate()
                    })
                    .addAction("5 Rows", {
                        for (row in 0..4) {
                    ...
                    .addAction("Column", {
                        ...
                        model.addColumn("Column " + (columnCounter++), labels.toTypedArray())
                        table.invalidate()
                    })
                    .build()
                    .showDialog(textGUI)
        ...
        buttonPanel.addComponent(
            Button("Remove...", {
                ActionListDialogBuilder()
                    ...
                    .addAction("Row", {
                        val numberAsText = askForANumber(textGUI, "Enter row # to remove (0-" + (model.getRowCount() - 1) + ")")
                        if (numberAsText != null) {
                            model.removeRow(Integer.parseInt(numberAsText))
                        }
                    })
                    .addAction("Column", {
                        val numberAsText = askForANumber(textGUI, "Enter column # to remove (0-" + (model.getColumnCount() - 1) + ")")
                        if (numberAsText != null) {
    ...
    private fun onModify(
        ...
        val dialogChoices = arrayOf("Change table content", "Change table style", "Change view size", "Force re-calculate/re-draw")
    ...
    private fun onModifyContent(
        ...
        val newLabel =
            askForAString(textGUI, "Enter new label for the table cell at row " + rowIndexAsText + " column " + columnIndexAsText)
        if (newLabel != null) {
            model.setCell(Integer.parseInt(columnIndexAsText), Integer.parseInt(rowIndexAsText), newLabel)
        }
    }

    private fun onModifyStyle(
        textGUI: WindowBasedTextGUI,
        table: Table<String>,
    ) {
        val dialogChoices =
        ...
        if (choice == null) {
            ...
            val newStyle =
                ListSelectDialogBuilder<TableCellBorderStyle?>()
