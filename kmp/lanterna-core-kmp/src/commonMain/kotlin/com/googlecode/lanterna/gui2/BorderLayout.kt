 ...
 * @author martin
 */
open class BorderLayout : LayoutManager {
    ...
    override open fun getPreferredSize(components: List<Component>): TerminalSize {
        val layout = makeLookupMap(components)
        val preferredHeight =
            (if (layout.containsKey(Location.TOP)) getRequired(layout, Location.TOP).getPreferredSize().getRows() else 0) +
                Math.max(
                    if (layout.containsKey(Location.LEFT)) getRequired(layout, Location.LEFT).getPreferredSize().getRows() else 0,
                    Math.max(
                        if (layout.containsKey(Location.CENTER)) getRequired(layout, Location.CENTER).getPreferredSize().getRows() else 0,
                        if (layout.containsKey(Location.RIGHT)) getRequired(layout, Location.RIGHT).getPreferredSize().getRows() else 0
                    )
                ) +
                (if (layout.containsKey(Location.BOTTOM)) getRequired(layout, Location.BOTTOM).getPreferredSize().getRows() else 0)

        val preferredWidth =
            Math.max(
                (if (layout.containsKey(Location.LEFT)) getRequired(layout, Location.LEFT).getPreferredSize().getColumns() else 0) +
                    (if (layout.containsKey(Location.CENTER)) getRequired(layout, Location.CENTER).getPreferredSize().getColumns() else 0) +
                    (if (layout.containsKey(Location.RIGHT)) getRequired(layout, Location.RIGHT).getPreferredSize().getColumns() else 0),
                Math.max(
                    if (layout.containsKey(Location.TOP)) getRequired(layout, Location.TOP).getPreferredSize().getColumns() else 0,
                    if (layout.containsKey(Location.BOTTOM)) getRequired(layout, Location.BOTTOM).getPreferredSize().getColumns() else 0
                )
            )
        return TerminalSize(preferredWidth, preferredHeight)
    }

    override open fun doLayout(area: TerminalSize, components: List<Component>) {
        val layout = makeLookupMap(components)
        var availableHorizontalSpace = area.getColumns()
        var availableVerticalSpace = area.getRows()
        ...
        if (layout.containsKey(Location.TOP)) {
            val topComponent = getRequired(layout, Location.TOP)
        ...
        if (layout.containsKey(Location.BOTTOM)) {
            val bottomComponent = getRequired(layout, Location.BOTTOM)
            val bottomComponentHeight = Math.min(bottomComponent.getPreferredSize().getRows(), availableVerticalSpace)
            bottomComponent.setPosition(TerminalPosition(0, area.getRows() - bottomComponentHeight))
        ...
        if (layout.containsKey(Location.LEFT)) {
            val leftComponent = getRequired(layout, Location.LEFT)
        ...
        if (layout.containsKey(Location.RIGHT)) {
            val rightComponent = getRequired(layout, Location.RIGHT)
            val rightComponentWidth = Math.min(rightComponent.getPreferredSize().getColumns(), availableHorizontalSpace)
            rightComponent.setPosition(TerminalPosition(area.getColumns() - rightComponentWidth, topComponentHeight))
        ...
        if (layout.containsKey(Location.CENTER)) {
            val centerComponent = getRequired(layout, Location.CENTER)
    ...
    private fun makeLookupMap(components: List<Component>): EnumMap<Location, Component> {
        ...
        val unassignedComponents: MutableList<Component> = ArrayList()
        for (component in components) {
            if (!component.isVisible()) {
                continue
            }
            if (component.getLayoutData() is Location) {
                map[component.getLayoutData() as Location] = component
            } else {
    ...
    override open fun hasChanged(): Boolean {
        //No internal state
        return false
    }

    private fun getRequired(layout: EnumMap<Location, Component>, location: Location): Component {
        val component = layout[location]
        if (component == null) {
    ...
    companion object {
        ...
        private val AUTO_ASSIGN_ORDER: List<Location> = Collections.unmodifiableList(
            Arrays.asList(
                Location.CENTER,
                Location.TOP,
                Location.BOTTOM,
