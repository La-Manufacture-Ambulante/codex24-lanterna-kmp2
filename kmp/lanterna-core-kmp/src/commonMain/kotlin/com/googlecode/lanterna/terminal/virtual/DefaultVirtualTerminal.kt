...
class DefaultVirtualTerminal
    ...
    constructor(private var internalTerminalSize: TerminalSize? = TerminalSize(80, 24)) : AbstractTerminal(), VirtualTerminal {
        ...
        ): ByteArray = javaClass.name.toByteArray()
        ...
        override fun getBufferCharacter(
            column: Int,
            row: Int,
        ): TextCharacter? {
        ...
        override fun forEachLine(
            startRow: Int,
            endRow: Int,
            bufferWalker: VirtualTerminal.BufferWalker?,
        ) {
            val emptyLine =
                object : VirtualTerminal.BufferLine {
        ...
        private fun correctCursor() {
            val terminalSize = requireNotNull(internalTerminalSize)
            cursorBufferPosition =
                requireNotNull(cursorBufferPosition).withColumn(
                    minOf(cursorBufferPosition!!.column, terminalSize.columns - 1),
                )
            cursorBufferPosition =
                requireNotNull(cursorBufferPosition).withRow(
