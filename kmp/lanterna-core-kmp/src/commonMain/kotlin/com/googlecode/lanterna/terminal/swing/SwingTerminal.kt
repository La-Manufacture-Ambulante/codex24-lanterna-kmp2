...
@Suppress("serial")
open class SwingTerminal : JComponent, IOSafeTerminal {
    ...
    @Synchronized
    override open fun getPreferredSize(): Dimension {
        return terminalImplementation.getPreferredSize()
    }

    @Synchronized
    override protected open fun paintComponent(componentGraphics: Graphics) {
    ...
    override open fun getInputMethodRequests(): InputMethodRequests {
        return inputMethodRequests
    }

    override protected open fun processInputMethodEvent(e: InputMethodEvent) {
        val iterator: AttributedCharacterIterator? = e.text
        for (i in 0 until e.committedCharacterCount) {
            val it = iterator ?: throw NullPointerException()
            terminalImplementation.addInput(KeyStroke(it.current(), false, false))
            it.next()
        }
    }

    override open fun enterPrivateMode() {
        terminalImplementation.enterPrivateMode()
    }

    override open fun exitPrivateMode() {
        terminalImplementation.exitPrivateMode()
    }

    override open fun clearScreen() {
        terminalImplementation.clearScreen()
    }

    override open fun setCursorPosition(x: Int, y: Int) {
        terminalImplementation.setCursorPosition(x, y)
    }

    override open fun setCursorPosition(position: TerminalPosition?) {
        terminalImplementation.setCursorPosition(position)
    }

    override open fun getCursorPosition(): TerminalPosition? {
        return terminalImplementation.getCursorPosition()
    }

    override open fun setCursorVisible(visible: Boolean) {
        terminalImplementation.setCursorVisible(visible)
    }

    override open fun putCharacter(c: Char) {
        terminalImplementation.putCharacter(c)
    }

    override open fun putString(string: String?) {
        terminalImplementation.putString(string)
    }

    override open fun enableSGR(sgr: SGR?) {
        terminalImplementation.enableSGR(sgr)
    }

    override open fun disableSGR(sgr: SGR?) {
        terminalImplementation.disableSGR(sgr)
    }

    override open fun resetColorAndSGR() {
        terminalImplementation.resetColorAndSGR()
    }

    override open fun setForegroundColor(color: TextColor?) {
        terminalImplementation.setForegroundColor(color)
    }

    override open fun setBackgroundColor(color: TextColor?) {
        terminalImplementation.setBackgroundColor(color)
    }

    override open fun getTerminalSize(): TerminalSize? {
        return terminalImplementation.getTerminalSize()
    }

    override open fun enquireTerminal(timeout: Int, timeoutUnit: TimeUnit?): ByteArray? {
        return terminalImplementation.enquireTerminal(timeout, timeoutUnit)
    }

    override open fun bell() {
    ...
    override open fun pollInput(): KeyStroke? {
        return terminalImplementation.pollInput()
    }

    override open fun readInput(): KeyStroke? {
    ...
    override open fun addResizeListener(listener: TerminalResizeListener?) {
        terminalImplementation.addResizeListener(listener)
    }

    override open fun removeResizeListener(listener: TerminalResizeListener?) {
