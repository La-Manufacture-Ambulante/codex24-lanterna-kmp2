...
if (restlen >= 3)
{ // Control sequences require a minimum of three characters
val esc = string.charAt(index)
val bracket = string.charAt(index + 1)
if (esc.toInt() == 0x1B && bracket == '[')
{ // escape & open bracket
len = 3 // esc,bracket and (later)terminator.
 //  digits or semicolons can still precede the terminator:
                for (i in 2 until restlen)
{
val ch = string.charAt(i + index)
...
|| (unicodeBlock === Character.UnicodeBlock.HALFWIDTH_AND_FULLWIDTH_FORMS && c.toInt() < 0xFF61))    //The magic number here is the separating index between full-width and half-width
}

/**
 * Given a character, is this character considered to be a Thai character?
 * @param c Character to test
 * @return `true` if the character is a Thai character
 */
 ...
 * `true` for CJK (Chinese, Japanese and Korean) characters. Notice that emoji generally takes up more than a
 ...
 * otherwise `false`
 */
     fun isCharDoubleWidth(c:Char):Boolean {
return isCharCJK(c)
}
 ...
 */
     fun isControlCharacter(c:Char):Boolean {
 ...
 */
     fun isPrintableCharacter(c:Char):Boolean {
 ...
 */
     fun getColumnWidth(s:String?):Int {
 ...
 * applies only when you have an alignment-based [TabBehaviour]
...
@JvmOverloads  fun getColumnIndex(s:String?, stringCharacterIndex:Int, tabBehaviour:TabBehaviour? = TabBehaviour.CONVERT_TO_FOUR_SPACES, firstCharacterColumnPosition:Int = -1):Int {
var index = 0
 ...
 */
     fun fitString(string:String?, availableColumnSpace:Int):String? {
...
{ // only if rest contains non-whitespace
linesToBeWrapped.addFirst(row!!.substring(characterIndex))
}
}
}
...
private fun mapCodesToIntegerArray(codes:Array<String?>):Array<Integer?> {
val result = arrayOfNulls<Integer?>(codes.size)
...
val controlCodeType = controlSequence.charAt(controlSequence.length() - 1)
controlSequence = controlSequence.substring(2, controlSequence.length() - 1)
val codes = mapCodesToIntegerArray(controlSequence.split(";"))
...
38 -> if (i + 2 < codes.size && codes[i + 1] == 5)
{
...
39 -> target!!.setForegroundColor(original!!.getForegroundColor())
48 -> if (i + 2 < codes.size && codes[i + 1] == 5)
{
...
49 -> target!!.setBackgroundColor(original!!.getBackgroundColor())
