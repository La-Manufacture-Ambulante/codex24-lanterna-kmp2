...
private var labelWidth:Integer? = null
...
currentBounds = currentBounds!!.withColumns(labelWidth!!.toInt())!!.withRows(wordWrapped!!.size())
}
 ...
 * @return Itself
 */
    @Synchronized  fun setForegroundColor(foregroundColor:TextColor?):Label {
 ...
 * @return Itself
 */
    @Synchronized  fun setBackgroundColor(backgroundColor:TextColor?):Label {
 ...
 * @return Itself
 */
    @Synchronized  fun addStyle(sgr:SGR?):Label {
 ...
 * @return Itself
 */
    @Synchronized  fun removeStyle(sgr:SGR?):Label {
 ...
 * @return Itself
 */
    @Synchronized  fun setLabelWidth(labelWidth:Integer?):Label {
 ...
 * integer setting the requested maximum width at what point word wrapping will begin
 */
     fun getLabelWidth():Integer? {
return labelWidth
}

@Override
protected fun createDefaultRenderer():ComponentRenderer<Label?>? {
return object:ComponentRenderer<Label?>() {
@Override
 fun getPreferredSize(Label:Label?):TerminalSize? {
return labelSize
}

@Override
 fun drawComponent(graphics:TextGUIGraphics?, component:Label?) {
...
{
linesToDraw = TerminalTextUtils.getWordWrappedText(graphics!!.getSize().getColumns(), component!!.lines).toArray(arrayOfNulls<String?>(0))
}

for (row in 0 until Math.min(graphics!!.getSize().getRows(), linesToDraw!!.size))
{
val line = linesToDraw!![row]
if (graphics!!.getSize().getColumns() >= labelSize!!.columns)
{
graphics!!.putString(0, row, line)
}
else
{
val availableColumns = graphics!!.getSize().getColumns()
